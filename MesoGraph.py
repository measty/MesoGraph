...
if __name__=='__main__':
    ...
    base_path = Path(r'D:\Results\TMA_results\test_run24')
    ...
    if len(dataset_list) == 1:
        split_dataset = True
        dataset, slide, Y, _ = mk_graphs(dataset_list[0], load_graphs=load_graphs, use_res=use_res)
    else:
        dataset, slide, Y, _ = mk_graphs(dataset_list[0], load_graphs=load_graphs, use_res=use_res)
        test_dataset, slide_t, Y_t, _ = mk_graphs(dataset_list[1], load_graphs=load_graphs, use_res=use_res) 
    ...
    for reps in range(3):
        ...
        for tr, te in tr_test_split:
            ...
            train, valid = train_test_split(tr,test_size=0.25,shuffle=True,stratify=np.array(Y)[tr])
            ...
            if model_type == 'branched':
                model = MesoBranched(dim_features=train_dataset[0].x.shape[1], dim_target=dim_target, layers=layers, dropout = dropout, pooling='mean',eps=100.0,train_eps=False, do_ls=do_ls)
            elif model_type == 'separate':
                model = MesoSep(dim_features=train_dataset[0].x.shape[1], dim_target=dim_target, layers=layers, dropout = dropout, pooling='mean',eps=100.0,train_eps=False, do_ls=do_ls)
            else:
                raise ValueError('model_type must be branched or separate')
            net = NetWrapper(model, loss_function=None, device=device, save_dir=base_path)
            ...
            if opt == "adam":
                ...
                scheduler = CyclicLR(optimizer,learning_rate,5*learning_rate,40*len(tr_loader),mode='exp_range',gamma=0.8,cycle_momentum=True)
