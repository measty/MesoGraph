    ...
    def forward(self, x, edge_index=None, edge_weight=None, batch=None):
        ...
        for layer in range(self.no_layers):
            ...
            if layer == 0:
                ...
                z = F.dropout(self.linears[layer](x), p=self.dropout, training=self.training)
                Z+=z
...
class MesoSep(torch.nn.Module):
    ...
    def make_subnet(self):
        ...
        if self.do_ls:
            ...
            return nn.ModuleDict({'first': first_h, 'linears': linears, 'ecnns': ecnns, 'ecs':ecs, 'ls': ls})
        return nn.ModuleDict({'first': first_h, 'linears': linears, 'ecnns': ecnns, 'ecs':ecs})
