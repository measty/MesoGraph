...
for core in core_list:
    ...
    with open(graph_path/(f'{core.stem.split("_")[0]}.pkl'), 'rb') as f:
        g = pickle.load(f)
    graph_dict = {'edge_index': g.edge_index.cpu().numpy()}#, 'feat_names': explainer_output['top_feats'], 'feat_importances': explainer_output['top_scores']}
    ...
    df = df.apply(lambda x: (x - x.quantile(0.05)) / (x.quantile(0.95) - x.quantile(0.05)))
