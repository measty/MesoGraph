...
def mk_graph(dataset="meso", mode="TMA", to_use=None, use_res=False):
    ...
    if read_files:
        ...
        for dets in det_list:
            ...
            if use_res:
                res_feats = np.load(
                    p.parent / "det_res_feats_snorm" / (dfs[-1].Parent.iloc[0] + ".npy")
                )
    ...
    for df in dfs:
        if (
            ...
            and dataset == "meso"
            and (
                (df.Parent.iloc[0] not in pres.Core.values)
                or pres[pres.Core == df.Parent.iloc[0]].Quality.values[0] > keep_level
            )
        ):
            continue
        if mode == "TMA":
            df = df[
                ["Parent", "Centroid X µm", "Centroid Y µm"] + to_use + ["label"]
            ]  # .dropna()
            df = df.fillna(df.mean())
        else:
            df = df[
                ["Parent", "Centroid X µm", "Centroid Y µm"]
                + to_use
                + ["label", "istumor"]
            ]  # .dropna()
            df = df.fillna(df.mean())
        ...
        if use_JL and df.Parent.iloc[0] in JL_labels.Core.values:
            df.label.iloc[0] = JL_labels[JL_labels.Core == df.Parent.iloc[0]].JL.values[
                0
            ]
        ...
        alpha = 0.25
        w = (1 - alpha) * np.array(
            [0.25, 0.25] + [0.5 * alpha / len(to_use)] * len(to_use)
        )
        ...
        if mode == "TMA":
            g.coords = toTensor(
                set_core_origin(
                    df[["Centroid X µm", "Centroid Y µm"]].to_numpy(),
                    g.core,
                    core_cents,
                    core_node,
                    core_width,
