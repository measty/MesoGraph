...
all_images_c, a_mean1, a_mean2 = center_ims(
    np.concatenate((sarc_images_h, ep_images_h), axis=0)
)
all_images_orig_c, ao_mean1, ao_mean2 = center_ims(
    np.concatenate((sarc_images, ep_images), axis=0)
)
...
# umap_orig = UMAP(n_components=2, metric='minkowski', metric_kwds={"p": 3}, n_neighbors=10, densmap=True, dens_lambda=0.4, repulsion_strength=1.0, n_epochs=500, target_weight=0.4)
umap_orig = UMAP(
    n_components=2,
    metric="cosine",
    n_neighbors=10,
...
umap_orig.fit(
    np.reshape(all_images_orig_c, (all_images.shape[0], -1)),
    y=np.concatenate((np.ones(len(components_S)), np.zeros(len(components_E))), axis=0),
)
...
scat_A = figure(
    width=500,
    tools=["tap", "wheel_zoom", "box_select", "lasso_select", "pan"],
    height=500,
)
# pca_S2_ds = ColumnDataSource(data={'x': pca_S2.transform(components_S)[:,0], 'y': pca_S2.transform(components_S)[:,1]})

pca_A2_ds = ColumnDataSource(
    data={
        "x": umap_orig.embedding_[:, 0],
        "y": umap_orig.embedding_[:, 1],
        "score": np.vstack((props_S["scores"], props_E["scores"])),
    }
)
scat_A.circle(
    x="x",
    y="y",
    fill_color=linear_cmap("score", "Turbo256", 0, 1),
    line_color=linear_cmap("score", "Turbo256", 0, 1),
    fill_alpha=0.5,
    line_alpha=0.5,
    source=pca_A2_ds,
)
...
def scatter_select_cb_A(attr, old, new):
    ...
    for i, slider in enumerate(all_column):
        slider.value = comps[0, i]
    # set the image to the selected cell
    source_A_real.data["image"] = [
        image_as_uint32(
            np.concatenate(
                (
                    np.mean(all_images[new, :, :, :], keepdims=False, axis=0),
                    255 * np.ones((im_size, im_size, 1), dtype=np.uint8),
                ),
                axis=2,
            )
...
sarc_dens_ds = ColumnDataSource(
    {"x": np.linspace(-10, 10, 100), "y": np.zeros((100, 1))}
)

# make a new plot to show the densities of the cell groups
dens_plot = figure(width=500, height=300, title="Densities")
dens_plot.line(
    "x", "y", source=ep_dens_ds, color="blue", line_width=2, legend_label="ep"
)
dens_plot.line(
    "x", "y", source=sarc_dens_ds, color="red", line_width=2, legend_label="sarc"
)
...
def pc_select_cb(attr, old, new):
    ...
    if len(new) > 0:
        ...
        x_range = np.linspace(
            np.min([np.min(components_A_E[:, comp]), np.min(components_A_S[:, comp])]),
            np.max([np.max(components_A_E[:, comp]), np.max(components_A_S[:, comp])]),
            100,
        )
...
pc_selector = MultiChoice(
    title="Select a Principal Component",
    value=[],
    options=[str(i) for i in range(n_comp)],
    max_items=1,
)
...
curdoc().add_root(l)
curdoc().title = "Cell Morpholoy Viewer"
