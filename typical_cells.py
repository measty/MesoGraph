...
def plot_typical_images(sarc_images, ep_images, props_S, props_E, clust="all"):
    ...
    def show_comps(decomp_E, decomp_S, decomp_name="comp"):
        ...
        for i in range(n_comp):
            ...
            plt.imshow(
                h_comp_to_rgb(
                    decomp_E.components_[i, :], save_path / f"E_{decomp_name}_{i}.png"
                )
            )  # , cmap='gray')
            ...
            plt.imshow(
                h_comp_to_rgb(
                    decomp_S.components_[i, :], save_path / f"S_{decomp_name}_{i}.png"
                )
            )  # , cmap='gray')
    ...
    plt.scatter(
        pca_E.transform(
            np.reshape(
                np.vstack((ep_images, sarc_images)),
                (ep_images.shape[0] + sarc_images.shape[0], -1),
            )
        ).T[i],
        pca_E.transform(
            np.reshape(
        ...
        c=np.vstack((props_E["scores"], props_S["scores"])),
        cmap="Reds",
    )
    ...
    if do_umap:
        ...
        kmeans_E = AgglomerativeClustering(
            n_clusters=n_clust, affinity="cosine", linkage="average"
        )
        kmeans_E.fit(pca_E.transform(np.reshape(ep_images, (ep_images.shape[0], -1))))
        kmeans_S = AgglomerativeClustering(
            n_clusters=n_clust, affinity="cosine", linkage="average"
        )
        kmeans_S.fit(
            pca_S.transform(np.reshape(sarc_images, (sarc_images.shape[0], -1)))
        )

        # calulate the mean image for each cluster
        cluster_ind_E = (
            kmeans_E.labels_
        )  # kmeans_E.predict(np.reshape(ep_images, (ep_images.shape[0], -1)))
        cluster_ind_S = (
            kmeans_S.labels_
        )  # kmeans_S.predict(np.reshape(sarc_images, (sarc_images.shape[0], -1)))
    ...
    for i in range(n_clust):
        im_E = h_comp_to_rgb(
            cluster_images_E[sorted_E[i]], save_path / f"cluster_E_{i}.png"
        )
        plt.subplot(1, 2, 1)
        plt.imshow(im_E)  # , cmap='gray')
        plt.title("E PC " + str(i))
        im_S = h_comp_to_rgb(
            cluster_images_S[sorted_S[i]], save_path / f"cluster_S_{i}.png"
        )
        plt.subplot(1, 2, 2)
    ...
    if False:
        ...
        for i in range(mask.shape[0]):
            for j in range(mask.shape[1]):
                if (i + 0.5 - mask.shape[0] / 2) ** 2 + (
                    j + 0.5 - mask.shape[1] / 2
                ) ** 2 > 11**2:
                    mask[i, j, :] = 255
...
if __name__ == "__main__":
    ...
    if False:
        ...
        for core in dets_list:
            ...
            for i, ann in enumerate(core_db.values()):
                ...
                geom = translate(
                    ann.geometry, -ann.geometry.centroid.x, -ann.geometry.centroid.y
                )
                ...
                theta = acos(
                    comp1[1]
                )  # angle between vector [0,1] and principal component
                im = patches[i, :]
                if i % 100 == -1:
                    plt.subplot(1, 2, 1)
                    plt.imshow(im)
                    plt.scatter(
                        np.array(geom.exterior.coords)[:, 0] + 36.5,
                        np.array(geom.exterior.coords)[:, 1] + 36.5,
                    )
                # mask out the pixels in im that aren't inside geometry using cv2
                mask = np.zeros(im.shape[:2], dtype=np.uint8)
                cv2.fillConvexPoly(
                    mask,
                    (np.array(geom.buffer(2.0).exterior.coords) + 36.5).astype(
                        np.int32
                    ),
                    1,
                )
        ...
        np.savez(
            res_path.parent / "sarc_images_b.npz",
            images=np.squeeze(sarc_images),
            scores=scores_S,
            areas=areas_S,
            circ=circ_S,
        )
        np.savez(
            res_path.parent / "ep_images_b.npz",
            images=np.squeeze(ep_images),
            scores=scores_E,
            areas=areas_E,
            circ=circ_E,
        )
    ...
    sarc_images = sarc_images[
        np.squeeze(
            np.logical_and(props_S["areas"] > limits[0], props_S["areas"] < limits[1])
        ),
        :,
    ]
    props_S = index_all_in_dict(
        props_S,
        np.where(
            np.logical_and(props_S["areas"] > limits[0], props_S["areas"] < limits[1])
        )[0],
    ...
    ep_images = ep_images[
        np.squeeze(
            np.logical_and(props_E["areas"] > limits[0], props_E["areas"] < limits[1])
        ),
        :,
    ]
    props_E = index_all_in_dict(
        props_E,
        np.where(
            np.logical_and(props_E["areas"] > limits[0], props_E["areas"] < limits[1])
        )[0],
    ...
    kmeans_S = KMeans(n_clusters=4, random_state=0).fit(
    ...
    kmeans_E = KMeans(n_clusters=4, random_state=0).fit(
        StandardScaler().fit_transform(X)
    )
    ...
    for i in range(4):
        ...
        plot_typical_images(
            sarc_images_h[idx_S, :],
            ep_images_h[idx_E, :],
            index_all_in_dict(props_S, idx_S),
            index_all_in_dict(props_E, idx_E),
            f"cluster_{i}",
        )

    # do lda discriminating between top 10% sarc images and bottom 10% ep images
    lda = LinearDiscriminantAnalysis(n_components=1)
    top_bot_images = np.vstack(
        (
            sarc_images_h[: int(0.1 * len(sarc_images_h)), :],
            ep_images_h[-int(0.1 * len(ep_images_h)) :, :],
        )
    )
    top_bot_labels = np.hstack(
        (np.ones(int(0.1 * len(sarc_images_h))), np.zeros(int(0.1 * len(ep_images_h))))
    )
